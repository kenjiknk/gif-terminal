...
# ============================================
# ANSI colour overrides — Arch-inspired palette
# ============================================
# Cool blue/cyan values keep the Arch theme distinct while still
# contrasting sharply against the dark frosted terminal glass.
...
# 2. Optional: assets/arch_wallpaper.png for a custom background
#
...
#   - Arch wallpaper or generated Arch background fills the canvas
#   - Frosted glass covers the window area
#   - Terminal content composited via chroma-key
#   - Lightweight Linux desktop chrome: title bar + menu bar + right-side buttons
# ============================================
...
MENU_H        = 22              # Linux terminal menu bar (File Edit View...)
...
CORNER_RADIUS = 6               # tighter radius than macOS
...
def _generated_arch_wallpaper():
    """Generate a simple Arch-inspired fallback wallpaper."""
    bg = Image.new("RGB", (GIF_W, GIF_H), (7, 18, 28))
    d = ImageDraw.Draw(bg, "RGBA")
    for y in range(GIF_H):
        t = y / max(1, GIF_H - 1)
        r = int(7 + 5 * t)
        g = int(18 + 34 * t)
        b = int(28 + 54 * t)
        d.line([(0, y), (GIF_W, y)], fill=(r, g, b, 255))

    # Abstract Arch-like angular mark, intentionally not the official logo.
    center_x = GIF_W // 2
    d.polygon(
        [
            (center_x, 70),
            (center_x - 180, GIF_H - 58),
            (center_x - 72, GIF_H - 58),
            (center_x - 26, GIF_H - 180),
            (center_x + 28, GIF_H - 180),
            (center_x + 76, GIF_H - 58),
            (center_x + 184, GIF_H - 58),
        ],
        fill=(23, 147, 209, 180),
    )
    d.polygon(
        [
            (center_x, 126),
            (center_x - 58, GIF_H - 128),
            (center_x + 58, GIF_H - 128),
        ],
        fill=(55, 182, 246, 90),
    )
    return bg.filter(ImageFilter.GaussianBlur(radius=0.4))


def prepare_arch_layers(wallpaper_path):
    """
    ...
    if os.path.exists(wallpaper_path):
        ...
        print(f"INFO: {wallpaper_path} not found — using generated Arch background.")
        wallpaper = _generated_arch_wallpaper()
    ...
    # Window title text (left-aligned, lightweight Linux style)
    title_font = _default_font(11)
    title_text = f"{USERNAME}@arch: ~"
    d.text(
    ...
    d.ellipse([(close_x-btn_r, btn_y-btn_r), (close_x+btn_r, btn_y+btn_r)],
              fill=(224, 108, 117, 235), outline=(255, 130, 138, 180)) # close
...
def post_process_frames(base_canvas, chrome, frames_dir=FRAMES_DIR):
    """Composite Arch chrome onto every PNG frame gifos generated."""
    frame_files = sorted(
        glob.glob(f"{frames_dir}/{FRAME_BASE}*.png"),
        key=lambda x: int(os.path.splitext(os.path.basename(x))[0].split("_")[1]),
    )
    print(f"INFO: Post-processing {len(frame_files)} frames with Arch Linux theme...")
...
# Colours that must survive GIF quantisation — UI chrome + all ANSI values.
_PALETTE_HINTS = [
    # Chrome
    (224, 108, 117),  # close button red
    ( 65,  65,  70),  # gray buttons
    (210, 210, 210),  # title text
    (200, 200, 200),  # menu text
    # ANSI overrides above
    (224, 108, 117),  (255, 107, 107),  # red / bright red
    ( 78, 170,  37),  (138, 226,  52),  # green / bright green
    (196, 160,   0),  (252, 233,  79),  # gold / bright yellow
    ( 23, 147, 209),  ( 55, 182, 246),  # Arch blue / bright blue
    (163, 122, 204),  (198, 120, 221),  # magenta / bright magenta
...
# Arch bash prompt: green user @ blue hostname : cyan path $
t.set_prompt(
    f"\x1b[92m{USERNAME}\x1b[0m@\x1b[94march\x1b[0m:\x1b[96m~\x1b[0m$ "
)
...
skills = [
    ("\x1b[94mMobile:\x1b[0m      ", "Android Java, Android SDK, AOSP/Framework"),
    ("\x1b[94mAndroid:\x1b[0m     ", "Lifecycle, Build/System, Debugging, Performance"),
    ("\x1b[94mBackend:\x1b[0m     ", "Java/Spring, Python, REST APIs, Node.js"),
    ("\x1b[94mFrontend:\x1b[0m    ", "React, HTML/CSS, JavaScript/TypeScript"),
    ("\x1b[94mDatabase:\x1b[0m    ", "SQL, PostgreSQL/MySQL, MongoDB, Data Modeling"),
    ("\x1b[94mLow-Level:\x1b[0m   ", "C, Java, Rust, Lua, Bash"),
    ("\x1b[94mQuality:\x1b[0m     ", "Git, GitHub, Tests, Clean Code, SOLID"),
    ("\x1b[94mDevOps:\x1b[0m      ", "Docker, GitHub Actions, CI/CD, Linux"),
    ("\x1b[94mTools:\x1b[0m       ", "Neovim, ADB, Gradle, Logcat, Shell, Postman"),
    ("\x1b[94mCS:\x1b[0m          ", "Algorithms, Data Structures, OOP"),
    ("\x1b[94mOS:\x1b[0m          ", "Arch Linux"),
]
...
# ============================================
# Post-process frames → Arch Linux theme
# ============================================

base_canvas, chrome = prepare_arch_layers("assets/arch_wallpaper.png")
